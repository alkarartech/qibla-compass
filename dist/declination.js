...
const DEFAULT_DECLINATION_URL = 'https://www.ngdc.noaa.gov/geomag-web/calculators/calculateDeclination';
/** Coarse approximate declination by latitude (degrees E). Only for fallback when API unavailable. */
function staticDeclinationByLat(lat, lon) {
 ...
 */
export async function getMagneticDeclination(lat, lon, options = {}) {
    ...
    try {
        ...
        const deg = typeof data.declination === 'number'
            ? data.declination
            : typeof data.declination_angle === 'number'
                ? data.declination_angle
                : NaN;
 ...
 */
export function getCachedDeclination(lat, lon, useStaticFallback = true) {
    const key = cacheKey(lat, lon);
...
export function clearDeclinationCache() {
    cache.clear();
}
